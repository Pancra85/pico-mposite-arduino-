...
void cls(unsigned char c) {
    memset(screen_bitmap, colour_base + c, height * width);
}
...
void scroll_up(unsigned char c, int rows) {
    memcpy(screen_bitmap, &screen_bitmap[width * rows], (height - rows) * width);
    memset(&screen_bitmap[width * (height - rows)], colour_base + c, rows * width);
}
...
void plot(int x, int y, unsigned char c) {
    if(x >= 0 && x < width && y >= 0 && y < height) {
        screen_bitmap_next[width * y + x] = colour_base + c;
    }
}
...
void draw_horizontal_line(int y1, int x1, int x2, int c) {
    ...
    memset(&screen_bitmap[width * y1 + x1], colour_base + c, x2 - x1 + 1);
}
...
void blit(const void * data, int sx, int sy, int sw, int sh, int dx, int dy) {
    ...
    void * dst = screen_bitmap + (width * dy) + dx;
    for(int i = 0; i < sh; i++) {
        memcpy(dst, src, sw);
        dst += width;
        src += sw;
